...
public class PdsController extends HttpServlet {
	 ...
	 * throws IOException { String f = fileItem.getName(); long sizeInBytes =
	 * fileItem.getSize();
	 * 
	 * String fileName = ""; String fpost = "";
	 * 
	 * // 업로드한 파일이 정상일 경우 if (sizeInBytes > 0) { if (f.indexOf('.') >= 0) { fpost =
	 * f.substring(f.indexOf('.')); fileName = new Date().getTime() + fpost; } else
	 * { fileName = new Date().getTime() + ".back"; } try { File uploadFile = new
	 * File(dir, fileName); fileItem.write(uploadFile); // 실제 업로드하는 부분 } catch
	...
	/*
	 * public String processUploadedFile(FileItem fileItem, String dir, JspWriter
	 * out) throws IOException{
	 * 
	 * String fileName = fileItem.getName(); long sizeInBytes = fileItem.getSize();
	 * 
	 * //업로드한 파일이 정상일 경우 if(sizeInBytes>0){ int idx = fileName.lastIndexOf("\\");
	 * 
	 * 만약 파일명이 c:\\temp\abc.jpg라면 뒤에서부터 가장 첫 \를 찾아 \뒤의 abc.jpg를 가지고 오기 위해서.
	 * 
	 * if(idx==-1){ // \ 못찾음. idx = fileName.lastIndexOf("/"); } fileName =
	 * fileName.substring(idx+1);//그 다음거도 다 가지고 와라. abc.jpg
	 * 
	 * try{ File uploadedFile = new File(dir, fileName); // c:\\temp\abc.jpg는 지정해둔
	 * 특정 구역에 들어 가고오있다. fileItem.write(uploadedFile);
	 * 
	 * 

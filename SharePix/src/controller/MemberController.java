			...
			try {
				...
				while (it.hasNext()) {
					...
					if (item.isFormField()) {
						...
						if (item.getFieldName().equals("profile_keep_or_default")) {
							profile_keep_or_default = item.getString("utf-8");
							System.out.println("item profile_keep_or_default = " + profile_keep_or_default);
						}else if(item.getFieldName().equals("fileload")){
							if(item.getName().equals(id) || item.getName()==null || item.getName().equals("")) { // 새로운 프로필 파일이 들어오지 않은 경우
								System.out.println("item.getName() 이 null : 기본 프로필 유지 혹은 삭제 : " + item.getName());

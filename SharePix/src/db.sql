...
CREATE VIEW PDSALL (SEQ, ID, CATEGORY, TAGS, UPLOADDATE, FILENAME, READCOUNT, DOWNCOUNT, FSAVENAME, LIKECOUNT, REPLYCOUNT, REPORT)
AS
SELECT P.SEQ, P.ID, P.CATEGORY, P.TAGS, P.UPLOADDATE, P.FILENAME, P.READCOUNT, P.DOWNCOUNT, P.FSAVENAME, 
	...
	NVL((SELECT COUNT(*) FROM PDSREPLY GROUP BY PDSSEQ HAVING PDSSEQ = P.SEQ),0),REPORT	
FROM PICPDS P;
...
INSERT INTO ALARM (SEQ, TOID, FROMID, ATYPE, PDSSEQ, ADATE,CONTENT) 
VALUES((SELECT NVL(MAX(SEQ),0)+1 FROM ALARM), 'dh2', 'dh', 1, 1, SYSDATE,'#123#123#123');
INSERT INTO ALARM (SEQ, TOID, FROMID, ATYPE, PDSSEQ, ADATE, CONTENT) 
VALUES((SELECT NVL(MAX(SEQ),0)+1 FROM ALARM), 'dh2', 'dh', 2, 1, SYSDATE,'아아아아');
